//! Cloudflare Root CA certificates, embedded from cloudflared source:
//! https://github.com/cloudflare/cloudflared/blob/master/tlsconfig/cloudflare_ca.go
//!
//! These are required because h2.cftunnel.com is signed by Cloudflare's own CA
//! which is not in the standard public trust stores.

/// All Cloudflare CA certs in PEM format (Origin SSL + Origin Pull)
pub const CLOUDFLARE_ROOT_CA_PEM: &[u8] = br#"
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
"#;

/// Parse and return all embedded Cloudflare CA certs as DER bytes
pub fn cloudflare_ca_certs() -> Vec<rustls::Certificate> {
    let mut certs = Vec::new();
    let mut remaining = CLOUDFLARE_ROOT_CA_PEM;
    loop {
        match rustls_pemfile::read_one(&mut remaining) {
            Ok(Some(rustls_pemfile::Item::X509Certificate(der))) => {
                certs.push(rustls::Certificate(der));
            }
            Ok(Some(_)) => continue,
            Ok(None) | Err(_) => break,
        }
    }
    certs
}
